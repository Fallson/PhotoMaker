...
void QtColorPicker::paintEvent(QPaintEvent *e)
{
    if (dirty) {
        ...
        QPixmap pix(iconSize + ANGEL_SIZE, iconSize);
		QPainter p(&pix);
		...
		QColor nocolor;
		nocolor.setAlpha(0);
		p.setBackground(QBrush(nocolor));
		if ( col.alpha() != 0)
		{
			...
			p.setBrush(col);
			p.drawRect(2, 2, w - 5 - ANGEL_SIZE, h - 5);
		}
		else
		{
			//pix.fill(QColor(qRgb(255,255,255)));
			...
			p.drawLine(2, 2, w - 5 - ANGEL_SIZE, h - 5);
			p.drawLine(2, h - 5, w - 5 -ANGEL_SIZE, 2);

		}
		p.setPen(QPen(Qt::black));
		p.setBrush(Qt::black);
		QPolygon triple;
		triple.push_back(QPoint(w-ANGEL_SIZE, 4));
		triple.push_back(QPoint(w-ANGEL_SIZE/2, h-2));
		triple.push_back(QPoint(w, 4));
		triple.push_back(QPoint(w-ANGEL_SIZE, 4));
		p.drawPolygon(triple);
		//p.drawLine(2, h - 5, w - 5, 2);
...
void ColorPickerItem::paintEvent(QPaintEvent *)
{
	...
	if ( 0 == c.alpha() )
	{
		p.drawLine(QPointF(3,3),QPointF(w-7, h-7));
		p.drawLine(QPointF(3, h-7),QPointF(w-7,3));
	}
	else
	{
